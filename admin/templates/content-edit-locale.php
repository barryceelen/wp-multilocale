 ...
 */
do_action( "{$locale_taxonomy_obj->name}_pre_edit_form", $locale_obj, $locale_taxonomy_obj->name );
?>
...
<form name="multilocale-edit-locale" id="multilocale-edit-locale" method="post" action="">
	...
	<input type="hidden" name="locale_id" value="<?php esc_attr_e( $locale_obj->term_id ); ?>" />
	<input type="hidden" name="action" value="edit_locale">
	<table class="form-table">
		<tbody>
			<tr>
				<th scope="row">
					<label for="name"><?php esc_html_e( 'Name', 'multilocale' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th scope="row">
					<label for="slug"><?php esc_html_e( 'Slug' ); ?></label>
				</th>
				<td>
					<input name="slug" id="slug" class="" type="text" value="<?php echo esc_attr( $locale_obj->slug ); ?>" />
					<p class="description"><?php esc_html_e( 'The &#8220;slug&#8221; is the URL-friendly version of the name. It is usually all lowercase and contains only letters, numbers, and hyphens.' ); ?>
					</p>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<th scope="row">
					<label for="blogname"><?php esc_html_e( 'Site Title' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th scope="row">
					<label for="blogdescription"><?php esc_html_e( 'Tagline' ); ?></label>
				</th>
				<td>
			...
			<tr>
			<th scope="row"><?php esc_html_e('Date Format') ?></th>
			<td>
				<fieldset><legend class="screen-reader-text"><span><?php esc_html_e('Date Format') ?></span></legend>
			<?php
				...
				*/
				...
				echo '<label for="date_format_custom" class="screen-reader-text">' . __( 'Custom date format:' ) . '</label><input type="text" name="date_format_custom" id="date_format_custom" value="' . esc_attr( $locale_date_format ) . '" class="" /> <span class="screen-reader-text">' . __( 'example:' ) . ' </span><span class="example"> ' . date_i18n( $locale_date_format ) . "</span> <span class='spinner'></span>\n";
			?>
				</fieldset>
			</td>
			</tr>
				...
				<td>
					<fieldset>
						...
						printf(
							...
							$locale_obj->term_id,
							$options['default_locale_id'] == $locale_obj->term_id ? ' checked="checked" disabled="disabled"' : '',
							$options['default_locale_id'] == $locale_obj->term_id ? sprintf( __( '&#8220;%s&#8221; is the default locale', 'multilocale' ), $locale_obj->name ) : sprintf( __( 'Make &#8220;%s&#8221; the default locale', 'multilocale' ), $locale_obj->name )
						);
						?>
						</label>
					</fieldset>
	 ...
	 */
	do_action( "{$locale_taxonomy_obj->name}_edit_form", $locale_obj, $locale_taxonomy_obj->name );
